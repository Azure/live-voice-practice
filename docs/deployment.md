...
| ACS media egress firewall rules (`enableAcsMediaEgress`) | Enabled in NI mode (basic mode has no firewall) | `azd env set ENABLE_ACS_MEDIA_EGRESS false` | Opens UDP 3478-3481 / TCP 443+3478-3481 to `AzureCloud` through Azure Firewall so the Live Voice Agent, ACS Calling, and Teams Media can stream. |
